...
object ControlFunction
{
    def forMaster(bot: Bot) {
        ...
        val dontFireEnergyGathererUntil = bot.inputAsIntOrElse("dontFireEnergyGathererUntil", -1)
        ...
        var direction = bot.view.center
        val dir = BFS(bot.view)
        if (!dir.isEmpty)
        {
            direction = bot.view.relPosFromAbsPos(dir.get)
            bot.move(direction)
        }
        else {
        ...
        if (dontFireEnergyGathererUntil < bot.time && bot.energy > 200) {
           bot.spawn(XY(1, -1), "mood" -> "Gather")
           bot.set("dontFireEnergyGathererUntil" -> (bot.time + 3))
        }
        if(dontPlantLandMineUntil < bot.time && bot.energy > 100) {
            bot.spawn(XY(1, 1), "mood" -> "Land")
            bot.set("dontPlantLandMineUntil" -> (bot.time + 100))
        }
    }
    ...
    def forSlave(bot: MiniBot) {
        bot.inputOrElse("mood", "Lurking") match {
            ...
            case "Gather" => reactAsGatherer(bot)
            case "Land" => reactAsLandMine(bot)
    ...
    def reactAsGatherer(bot: MiniBot) {
        val rnd = new Random()
        if (bot.energy > 1000) {
            val dir = bot.offsetToMaster
                bot.move(dir.signum)
        }
        else if (bot.energy < 1000) {
            bot.view.offsetToNearest('B') match {
                case Some(delta: XY) =>
                ...
                bot.view.offsetToNearest('P') match {
                    ...
                    bot.move(delta.signum)
                    case None =>
                    bot.move(XY(rnd.nextInt(3)-1, rnd.nextInt(3)-1))
                }
            }
    ...
    def BFS(view : View) : Option[XY] = {
        var checked = (1 to view.size*view.size map(_ => false)).toArray

        var parent = (1 to view.size*view.size map(_ => XY(0,0))).toArray
        ...
        checked(view.indexFromAbsPos(center)) = true
        
        while (!queue.isEmpty) {
            val value = queue.dequeue
            val cellVal = view.cellAtAbsPos(value)

            if (cellVal == 'P' || cellVal == 'B') {
                var found = value
                while (parent(view.indexFromAbsPos(found)) != center)
                    found = parent(view.indexFromAbsPos(found))
                return Some(found)
            }
            
         val nextvals = Array(XY.Right, XY.RightUp,XY.Up, XY.UpLeft,XY.Left,XY.LeftDown, XY.Down, XY.DownRight)

                for (next <- nextvals) {
                    if (value.x + next.x > 0 && value.y + next.y > 0 && value.x + next.x < view.size && value.y + next.y < view.size) {
                val neighbor = value + XY(next.x,next.y)
                val neighborCell = view.cellAtAbsPos(neighbor)
                val neighborIndex = view.indexFromAbsPos(neighbor)
                
                if (!checked(neighborIndex)) {
                    checked(neighborIndex) = true
                    
                    if (neighborCell != 'W' && neighborCell != 'p') {
                        parent(neighborIndex) = value
